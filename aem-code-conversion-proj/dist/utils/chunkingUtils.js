...
exports.splitMethodIntoChunks = splitMethodIntoChunks;
const tokenEstimator_1 = require("./tokenEstimator");
exports.TOKEN_LIMIT = 4096; // Token limit for OpenAI
/**
 ...
 */
function chunkMethodsByTokenLimit(metadata, fileContent) {
    const chunks = [];
    ...
    metadata.methods.forEach(method => {
        ...
        const tokenCount = (0, tokenEstimator_1.estimateTokens)(methodContent);
        console.log('tokenCount in chunkMethodsByTokenLimit   ', tokenCount + ' - ' + method.methodName);
        if (tokenCount <= exports.TOKEN_LIMIT) {
 ...
 * @returns Array of strings, each representing a logical chunk.
 */
function splitMethodIntoChunks(methodContent) {
    console.log('inside splitMethodIntoChunks');
    const chunkSize = exports.TOKEN_LIMIT / 2; // Keep chunks well within token limit
    const methodChunks = [];
    let currentChunk = "";
    methodContent.split(/\n/).forEach(line => {
        const estimatedTokens = (0, tokenEstimator_1.estimateTokens)(currentChunk + line);
        if (estimatedTokens > chunkSize) {
...
//# sourceMappingURL=chunkingUtils.js.map